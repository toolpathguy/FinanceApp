...
/**
 * Build the envelope budget report: Ready to Assign + per-category
 * Assigned/Activity/Available, grouped. The single source of this computation,
 * shared by `GET /api/budget` and the AI `get_budget` tool (Issue #8) so the
 * report and the assistant can never disagree.
 *
 * @param period A validated hledger period expression, or '' for all-time.
 *   Callers (the route, the tool) validate/normalize before calling — this
 *   function assumes `period` is safe to pass to hledger.
 */
export async function getBudgetReport(period: string): Promise<BudgetEnvelopeReport> {
  const pd = period
